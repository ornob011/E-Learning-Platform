 ...
 * @author
 */
public class Enroll_NewStudentController implements Initializable {
    ...
    @FXML
    private Button btn_enroll_student;
    ...
    @FXML
    private TableColumn<CourseDetails, String> col_studentname;
    /**
...
//        }
    ...
    @FXML
    private void enrollStudentButtonAction(MouseEvent event) {
        ...
        if (courseName.equals("")
            ...
            String update2 = "INSERT INTO payment(courseName,courseID,studentName,regNo)" + "VALUES(?,?,?,?)";

            String update3 = "INSERT INTO parent_student(fatherName,studentName,tutorName)" + "VALUES(?,?,?)";

            connection = DBHandler.connectDB();
            try {
                pst = connection.prepareStatement(update);
                pst2 = connection.prepareStatement(update2);
                pst3 = connection.prepareStatement(update3);
            } catch (SQLException ex) {
                ex.printStackTrace();
            }
            try {
                ...
                pst2.setString(3, add_studentName.getText());
                pst2.setString(4, add_reg.getText());
                pst2.executeUpdate();

                pst3.setString(1, add_fathername.getText());
                pst3.setString(2, add_studentName.getText());
                pst3.setString(3, add_tutorname.getText());
                pst3.executeUpdate();

                JOptionPane.showMessageDialog(null, "Successfully Added New Student!!");
                autoRefresh();
            } catch (SQLException ex) {
                Logger.getLogger(Enroll_NewStudentController.class.getName()).log(Level.SEVERE, null, ex);
